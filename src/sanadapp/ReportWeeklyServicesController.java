 ...
 * @author Bassam
 */
public class ReportWeeklyServicesController implements Initializable {
    ...
    public void btSearchHandler(ActionEvent event) throws IOException {
        query = " select c.stating_center, COUNT(c.stating_center) as[العدد] from cases c where ";
        ...
        if (dp_startDate.getValue() == null && dp_endDate.getValue() == null) {
            ...
            query += " DATEDIFF(DAY,c.date,'" + dp_startDate.getValue() + "')<=0 ";
            q2 += " DATEDIFF(DAY,form_add_date,'" + dp_startDate.getValue() + "')<=0 ";
        } else if (dp_startDate.getValue() == null) {
            query += " DATEDIFF(DAY,c.date,'" + dp_endDate.getValue() + "')>=0 ";
        ...
        query += "and stating_center not like ''";
        query += " group by c.stating_center ";
    ...
    public void btPrintHandler(ActionEvent event) throws IOException {
        try {
            if (query != null) {
                Excel.print_as_excel(query, "تقرير الخدمات الأسبوعي");
            }
        } catch (SQLException ex) {
